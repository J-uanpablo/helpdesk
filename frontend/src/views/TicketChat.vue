...
const canConnect = computed(
  () => !!ticketId.value && token.value.trim().length > 0
);
...
async function loadTickets() {
  ...
  try {
    ...
    // si no hay ticket seleccionado y la lista trae algo, seleccionamos el primero
...
<template>
  <div
    class="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center pt-16"
  >
    <h1 class="text-4xl md:text-5xl font-extrabold mb-10 text-center">
      HD Panel de chat
    </h1>

    <!-- Panel de conexión -->
    <div
      class="w-full max-w-3xl bg-slate-800/70 border border-slate-700 rounded-xl p-6 mb-10 shadow-lg"
    >
    ...
    <!-- Área de chat -->
    <div
      class="w-full max-w-3xl bg-slate-800/70 border border-slate-700 rounded-xl p-6 shadow-lg mb-10"
    >
